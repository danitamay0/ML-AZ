...
# Creamos una variable que guarde el numero optimo de variables independientes, estadisticamente significativa

x_opt = x[:, [0,1,2,3,4,5] ]

# PASO 2 Creamos otro modelo de regresion
# Ordenary Least Squareds
regressor_OLS = sm.OLS(y, x_opt.tolist()).fit()
summary =  regressor_OLS.summary()


# PASO 3 Buscamos la variable predictora con el pvalor mas grande, si el pvalor es mas grande que SSL, la eliminamos del modelo
x_opt = x[:, [0,1, 3,4,5] ]
...
# Iteracion 4 // NOTA: como podemos ver quedó una regresion lineal simple. para ello no es optimp aunque su pvalor era 0.060,
# Para ello se pueden usar tecnicas de criterio de eliminacion mas avanzadas. Criterio de acaike o valeciano
x_opt = x[:, [0,3] ]
regressor_OLS = sm.OLS(y, x_opt.tolist()).fit()
summary2 =  regressor_OLS.summary()

y_predict_opt = regressor_OLS.predict()
